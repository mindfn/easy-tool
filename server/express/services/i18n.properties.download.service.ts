...
export default {
  /** 
   * @Author: zhuxiankang 
   * @Date:   2018-10-08 15:43:39  
   * @Desc:   多语言下载properties文件 
   * @Parm:    
   */  
  processDownloadProperties(i18nData: any, file:string, res: Response) {
  ...
  downloadAllLanguage(i18nData: any, file: string, res: Response) {
    try {
      ...
      // 将多语言数据写入各个语言类型对应的properties文件
      for(let language of Object.keys(i18n)) {
        let languagePath = path.join(i18nDirPath, `${EXPRESS.I18N_FILE_NAMES[language]}/${file||'index'}.properties`)
